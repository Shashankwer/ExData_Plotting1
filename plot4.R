...
plot( strptime(paste(d1$Date,d1$Time),"%d/%m/%Y %H:%M:%S"),as.numeric(d1$Voltage),type = "l",xlab  = 'datetime', ylab = 'Voltage')
...
legend("topright",legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"), lty = c(1,1,1), col = c("black","blue","red"),border="",bty="n")
plot( strptime(paste(d1$Date,d1$Time),"%d/%m/%Y %H:%M:%S"),as.numeric(d1$Global_reactive_power),type = "l",xlab  = 'datetime', ylab = 'Global_reactive_power')
dev.off()